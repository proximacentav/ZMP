...
bool zmpInstalledAsApp()
{
    ...
    // Нестандартное имя из прошлой установки сохраняется в конфиге
...
// Все привилегированные команды установки идут через sudo -S с паролем,
// который запрашивается у пользователя ОДИН раз (m_sudoPassword) и стирается
// из памяти после завершения установки/обновления.
    ...
    : QDialog(parent)
{
    ...
    if (mode == Mode::Update) {
        ...
        // Хэши не зависят от имени бинарника — только от содержимого
...
QString ZmpInstallDialog::installedBinaryPath()
{
    ...
    return QFileInfo::exists("/usr/bin/zmp") ? QStringLiteral("/usr/bin/zmp") : QString();
}
...
{
    // sudo-пароль запрашивается один раз на всю установку/обновление
    if (!ensureSudoPassword())
        return;

    // Имя: без пробелов, только латиница/цифры/_/- ; пустое -> zmp
    ...
    // 1. Копируем бинарник в /usr/bin/<name>
    ...
    // 4. Ассоциации файлов по умолчанию
    for (const QString &mime : mimeTypes)
        QProcess::execute("xdg-mime", {"default", name + ".desktop", mime});

    // 5. Ярлык на рабочий стол (все распространённые варианты пути)
    ...
    // Пароль стирается из памяти после завершения
    wipeSudoPassword();
    accept();
}
